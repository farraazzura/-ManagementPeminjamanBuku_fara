...
<body>
  <header>
    <h1>Dashboard - Manajemen Buku</h1>
  </header>
  <div class="container">
    <!-- Pencarian, Tambah Data dan Logout (di bawah teks Daftar Buku) -->
    <h2>Daftar Buku</h2>
    <div class="action-buttons">
      <!-- Pencarian -->
      <form action="{{ route('bukus.index') }}" method="GET" class="search-form">
        <input type="text" name="search" placeholder="Cari Buku... (Nama Pengarang / Judul Buku)" value="{{ request('search') }}">
        <button type="submit">Cari</button>
      </form>

      <!-- Tambah Data -->
      <a href="{{ route('bukus.create') }}">
        <button class="add-data-button" type="button">Tambah Data</button>
      </a>
    ...
    <div style="text-align: center; margin-top: 20px;">
      <a href="{{ route('dashboard.index') }}">
        <button class="back-button" type="button">Kembali ke Dashboard</button>
      </a>
    </div>
          ...
          <tr>
            ...
            <td class="actions">
              ...
              <form action="{{ route('bukus.destroy', $buku->id) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
                <button type="submit" class="delete" onclick="return confirm('Yakin ingin menghapus buku ini?')">Hapus</button>
              </form>
              <a href="{{ route('bukus.print', $buku->id) }}">
                <button class="print" type="button">Cetak</button>
              </a>
            </td>
          </tr>
        @empty
